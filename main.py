...
class KaraokePlayer:
    def __init__(self, stdscr):
        ...
        # --- Timing Fix: Use real audio position ---
        self.playback_start_time = 0  # Real time when music started
        self.seek_offset = 0.0        # Where in the song we are (in seconds)
        self.last_update_time = 0     # Last time we updated current_time
    ...
    def current_time(self):
        ...
        if pygame.mixer.music.get_busy():
            # Get how many milliseconds have passed since we started playing
            pos_ms = pygame.mixer.music.get_pos()  # Can be -1 if paused
            if pos_ms >= 0:
                return self.seek_offset + pos_ms / 1000.0
        
        # Fallback: use elapsed since last start
    ...
    def draw_current_line_progress(self, line, y, x):
        ...
        next_line_time = self.lyrics[self.current_line_idx + 1][0] if self.current_line_idx < len(self.lyrics) - 1 else self.total_time
    ...
    def handle_input(self, key):
        if key == ord('q'):
            ...
            if self.paused:
                ...
                self.paused = True
                # Update seek_offset to current time
    ...
    def run(self):
        ...
        while True:
            ...
            # Update current line based on accurate time
